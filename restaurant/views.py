...
def pos_save_draft(request):
    """Save a POS draft order to the database."""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'POST required'}, status=405)
    try:
        import json as _json
        ...
        if not draft_id:
            ...
            draft = PosDraft.objects.create(
                ...
                table_name=table, items_json=_json.dumps(items),
...
def pos_get_drafts(request):
    """Return all active POS drafts."""
    import json as _json
    drafts = PosDraft.objects.filter(is_deleted=False).order_by('-created_at')[:50]
    return JsonResponse({'drafts': [
        {'id': d.id, 'draft_number': d.draft_number, 'customer_name': d.customer_name,
         'table_name': d.table_name, 'total_amount': float(d.total_amount),
         'items': _json.loads(d.items_json), 'discount_pct': float(d.discount_pct),
         'subtotal': float(d.subtotal), 'note': d.note,
...
@login_required(login_url='/staff/login/')
def pos_export_excel(request):
    """Export all POS draft orders to Excel."""
    import json as _json
    ...
    drafts = PosDraft.objects.filter(is_deleted=False).order_by('-created_at')

    wb = Workbook()
    ws = wb.active
    ws.title = "POS Draft Orders"

    # Header styling
    hdr_fill = PatternFill("solid", fgColor="1a1a2e")
    hdr_font = Font(color="FFFFFF", bold=True)

    headers = ['Draft #', 'Date', 'Customer', 'Phone', 'Table', 'Items', 'Subtotal', 'Discount %', 'Total', 'Note']
    ...
    col_widths = [14, 18, 18, 14, 14, 40, 12, 12, 12, 25]
    for i, w in enumerate(col_widths, 1):
        ws.column_dimensions[ws.cell(row=1, column=i).column_letter].width = w

    for row, d in enumerate(drafts, 2):
        items = _json.loads(d.items_json)
        items_str = ', '.join([f"{i.get('qty',1)}x {i.get('name','?')}" for i in items])
        ws.append([
            d.draft_number,
    ...
    # Total row
    ws.append([])
    total_row = drafts.count() + 3
    ws.cell(row=total_row, column=8, value='GRAND TOTAL').font = Font(bold=True)
    ws.cell(row=total_row, column=9, value=float(sum(d.total_amount for d in drafts))).font = Font(bold=True)

    buf = BytesIO()
    wb.save(buf)
    buf.seek(0)
    resp = HttpResponse(buf, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    resp['Content-Disposition'] = f'attachment; filename="pos_drafts_{timezone.now().strftime("%Y%m%d")}.xlsx"'
    return resp
